...
static void ST7735_WriteChar(uint16_t x, uint16_t y, char ch, FontDef font, uint16_t color, uint16_t bgcolor) {
  uint32_t i, b, j, k, idx, fontHeightSize, radio , scanWidth;
  radio = (font.width - 1) / 8 + 1;
  fontHeightSize = radio * font.height;
  scanWidth = 	font.width < 8 ? font.width : 8;

  ST7735_SetAddressWindow(x, y, x +  font.width - 1, y + font.height - 1);

  for (i = 0; i < fontHeightSize; i+=radio) {
    for(k = 0 ; k < radio; k++){
      idx = (ch - 32) * fontHeightSize + i + k;
	  b = font.data[idx];
//	  printf("idx %d b %x\n",idx, b);
