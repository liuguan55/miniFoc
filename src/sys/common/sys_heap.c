...
int sys_heap_default_init(sys_heap_t *sysHeap, uint8_t *baseAddr, size_t totalSize)
{
    ...
    sysHeap->pxEnd = NULL;
    sysHeap->configTotal_Heap_Size = totalSize;
...
int sys_heap_init(sys_heap_t *sysHeap)
{
    ...
    xTotalHeapSize = sysHeap->configTotal_Heap_Size;
    ...
    malloc_mutex_lock();
    {
        ...
        if ((size & sysHeap->xBlockAllocatedBit) == 0) {
            ...
            if ((size > 0) && (size <= sysHeap->xFreeBytesRemaining)) {
                ...
                if ((pxBlock == pxBlockjudge) && (size < pxBlock->xBlockSize) && (size >= oldSize) &&
                    ((pxBlock->xBlockSize - size) > (sysHeap->heapMinmun_Block_Size))) {
