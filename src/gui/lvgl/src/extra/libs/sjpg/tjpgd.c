...
)
{
	...
	for (i = 0; i < 8; i++) {
		...
		/* Descale the transformed values 8 bits and driver a row */
#if JD_FASTDECODE >= 1
...
)
{
	...
	for (blk = 0; blk < nby + 2; blk++) {	/* Get nby Y blocks and two C blocks */
		...
		if (cmp && jd->ncomp != 3) {		/* Clear C blocks if not exist (monochrome image) */
			...
			if (JD_FORMAT != 2 || !cmp) {	/* C components may not be processed if in grayscale driver */
...
/*-----------------------------------------------------------------------*/
/* Output an MCU: Convert YCrCb to RGB and driver it in RGB form         */
/*-----------------------------------------------------------------------*/

static JRESULT mcu_output (
	JDEC* jd,			/* Pointer to the decompressor object */
	int (*outfunc)(JDEC*, void*, JRECT*),	/* RGB driver function */
...
)
{
	...
	if (!JD_USE_SCALE || jd->scale != 3) {	/* Not for 1/8 scaling */
		...
		if (JD_FORMAT != 2) {	/* RGB driver (build an RGB MCU from Y/C component) */
			for (iy = 0; iy < my; iy++) {
				pc = py = jd->mcubuf;
		...
		} else {	/* Monochrome driver (build a grayscale MCU from Y comopnent) */
			for (iy = 0; iy < my; iy++) {
				py = jd->mcubuf + iy * 8;
				if (my == 16) {		/* Double block height? */
					if (iy >= 8) py += 64;
			...
			for (iy = 0; iy < my; iy += w) {
				for (ix = 0; ix < mx; ix += w) {
					...
					for (y = 0; y < w; y++) {	/* Accumulate RGB value in the square */
						for (x = 0; x < w; x++) {
							r += *pix++;	/* Accumulate R or Y (monochrome driver) */
							if (JD_FORMAT != 2) {	/* RGB driver? */
								g += *pix++;	/* Accumulate G */
								b += *pix++;	/* Accumulate B */
							}
						}
					...
					*op++ = (uint8_t)(r >> s);	/* Put R or Y (monochrome driver) */
					if (JD_FORMAT != 2) {	/* RGB driver? */
...
)
{
	...
	for (;;) {				/* Parse JPEG segments */
		...
		case 0xDA:	/* SOS - Start of Scan */
			...
			/* Allocate working buffer for MCU and pixel driver */
			n = jd->msy * jd->msx;						/* Number of Y blocks in the MCU */
			if (!n) return JDR_FMT1;					/* Err: SOF0 has not been loaded */
			len = n * 64 * 2 + 64;						/* Allocate buffer for IDCT and RGB driver */
			if (len < 256) len = 256;					/* but at least 256 byte is required for IDCT */
			jd->workbuf = alloc_pool(jd, len);			/* and it may occupy a part of following MCU working buffer for RGB driver */
...
)
{
	...
	for (y = 0; y < jd->height; y += my) {		/* Vertical loop of MCUs */
		for (x = 0; x < jd->width; x += mx) {	/* Horizontal loop of MCUs */
			...
			if (rc != JDR_OK) return rc;
			rc = mcu_output(jd, outfunc, x, y);	/* Output the MCU (YCbCr to RGB, scaling and driver) */
			if (rc != JDR_OK) return rc;
		}
	}
