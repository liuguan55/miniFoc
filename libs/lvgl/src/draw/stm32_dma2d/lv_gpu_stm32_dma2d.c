...
#if LV_COLOR_16_SWAP
    // TODO: F7 has red blue swap bit in control register for all layers and driver
...
/**
 * Turn on the peripheral and set driver color mode, this only needs to be done once
 */
                                           ...
                                           lv_color_t color)
{
    ...
    /*as input color mode is same as driver we don't need to convert here do we?*/
                                          ...
                                          const lv_color_t * src_buf, lv_coord_t src_stride, lv_opa_t opa)
{
    ...
    if(opa >= LV_OPA_MAX) {
        DMA2D->CR = 0;
        /*copy driver colour mode, this register controls both input and driver colour format*/
