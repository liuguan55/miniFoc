...
//    lv_obj_add_flag(border, LV_OBJ_FLAG_IGNORE_LAYOUT);
    lv_obj_t *border = lv_obj_create(btn);
    lv_obj_set_size(border,LV_PCT(120), 2);
    lv_obj_set_style_bg_opa(border, LV_OPA_COVER, 0);
    lv_obj_set_style_bg_color(border, lv_palette_lighten(LV_PALETTE_ORANGE,3),0);
    lv_obj_set_style_bg_grad_color(border, lv_palette_lighten(LV_PALETTE_YELLOW,1),0);
    lv_obj_set_style_bg_grad_dir(border,LV_GRAD_DIR_HOR, 0 );
    lv_obj_set_style_radius(border, LV_RADIUS_CIRCLE, 0);
    lv_obj_align(border, LV_ALIGN_BOTTOM_MID, 0 , 0);
    lv_obj_set_style_bg_main_stop(border, 128, 0);
    lv_obj_set_style_bg_grad_stop(border, 192, 0);
    lv_obj_add_flag(border, LV_OBJ_FLAG_IGNORE_LAYOUT);
