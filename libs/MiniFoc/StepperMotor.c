...
void StepperMotor_move(Motor_t *motorBase, float new_target)
{
  ...
  switch (motorBase->controller) {
	...
	case Type_velocity_openloop:
	  ...
	  ctx->shaft_velocity_sp = new_target;
	  ctx->current_sp = StepperMotor_velocityOpenloop(motorBase, ctx->shaft_velocity_sp); // returns the voltage that is set to the motor
	  break;
	case Type_angle_openloop:
	  // angle control in open loop
	  ctx->shaft_angle_sp = new_target;
	  ctx->current_sp = StepperMotor_angleOpenloop(motorBase, ctx->shaft_angle_sp); // returns the voltage that is set to the motor
	  break;
  }
}
