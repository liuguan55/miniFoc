...
void BLDCMotor_initFOC(Motor_t *motorBase, float zero_electric_offset, Direction _sensor_direction)
{
  ...
  //shaft_angle update
  shaftVelocity(&motorBase->LPF_velocity, sensor->getVelocity(sensor), config->sensor_direction);  //getVelocity(),make sure velocity=0 after power on
  MiniFoc_delayMs(50);
  ctx->shaft_velocity =  shaftVelocity(&motorBase->LPF_velocity, sensor->getVelocity(sensor), config->sensor_direction); ;  //必须调用一次，进入主循环后速度为0
  MiniFoc_delayMs(5);
  ctx->shaft_angle = shaftAngle(sensor->getAngle(sensor), config->sensor_offset, config->sensor_direction);// shaft angle
...
int BLDCMotor_alignSensor(Motor_t *motorBase)
{
  ...
  if (config->zero_electric_angle == 0)  //没有设置，需要检测
  {
	...
	config->zero_electric_angle = _normalizeAngle(_electricalAngle(config->sensor_direction * angle, config->pole_pairs));
	MiniFoc_delayMs(20);
...
void BLDCMotor_loopFOC(Motor_t *motorBase)
{
  ...
  ctx->shaft_angle = shaftAngle(sensor->getAngle(sensor), config->sensor_offset, config->sensor_direction);;// shaft angle
  ctx->electrical_angle = electricalAngle(ctx->shaft_angle, config->pole_pairs, config->sensor_offset, config->zero_electric_angle);// electrical angle - need shaftAngle to be called first
...
void BLDCMotor_move(Motor_t  *motorBase, float new_target)
{
  ...
  if (MiniFoc_millis() - start > 3000){
	...
	log_i("shaft_angle %4.2f shaft_velocity %4.2f current_sp %4.2f",ctx->shaft_angle,ctx->shaft_velocity,ctx->current_sp);
  }

  float now_angle,angle_error;
  switch (motorBase->controller) {
	case Type_torque:
	  ctx->current_sp = new_target; // if current/foc_current torque control
	  break;
	case Type_spring:
	  // angle set point
	  ...
	  ctx->current_sp = LPFoperator(&motorBase->LPF_spring, PIDoperator(&motorBase->P_spring, (new_target - now_angle))); // if voltage torque control
	  break;
	case Type_springWithDump:
	  ...
	  // calculate the torque command
	  ctx->current_sp = LPFoperator(&motorBase->LPF_spring, PIDoperator(&motorBase->P_springWithDump, (new_target - now_angle))); // if voltage torque control
	  break;
	case Type_dump:
	  // angle set point
	  new_target = _normalizeAngle(new_target);
	  ...
	  // calculate the torque command
	  ctx->current_sp = LPFoperator(&motorBase->LPF_spring, PIDoperator(&motorBase->P_dump, (new_target - now_angle))); // if voltage torque control
	  break;
	case Type_zero_resistance:
	  ...
	  // calculate the torque command
	  ctx->current_sp = LPFoperator(&motorBase->LPF_spring, PIDoperator(&motorBase->P_zero_resistance, (new_target - now_angle))); // if voltage torque control
	  break;
	case Type_KNOB:
	  // angle set point
	  now_angle = _normalizeAngle(ctx->shaft_angle);
	...
	case Type_angle:
	  ...
	  // calculate the torque command
	  ctx->current_sp = PIDoperator(&motorBase->PID_velocity, (ctx->shaft_velocity_sp - ctx->shaft_velocity)); // if voltage torque control
	  break;
	case Type_velocity:
	  // velocity set point
	  ctx->shaft_velocity_sp = new_target;
	  // calculate the torque command
	  ctx->current_sp = PIDoperator(&motorBase->PID_velocity, (ctx->shaft_velocity_sp - ctx->shaft_velocity)); // if current/foc_current torque control
	  break;
	case Type_velocity_openloop:
	  // velocity control in open loop
	  ctx->shaft_velocity_sp = new_target;
	  ctx->current_sp = BLDCMotor_velocityOpenloop(motorBase, ctx->shaft_velocity_sp); // returns the voltage that is set to the motor
	  break;
	case Type_angle_openloop:
	  // angle control in open loop
	  ctx->shaft_angle_sp = new_target;
	  ctx->current_sp = BLDCMotor_angleOpenloop(motorBase, ctx->shaft_angle_sp); // returns the voltage that is set to the motor
	  break;
  }
}
