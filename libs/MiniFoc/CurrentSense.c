...
float getDCCurrent(FocCurrent_t *sense, float motor_electrical_angle)
{
	...
	current = sense->getPhaseCurrents();
	...
	if(motor_electrical_angle)sign = (i_beta * MINI_COS(motor_electrical_angle) - i_alpha*MINI_SIN(motor_electrical_angle)) > 0 ? 1 : -1;
...
DQCurrent_s getFOCCurrents(FocCurrent_t *sense,float angle_el)
{
	...
	ct = MINI_COS(angle_el);
	st = MINI_SIN(angle_el);
