...
int MiniFoc_setDefaultConfig(BLDCConfig_t *config) {
  ...
  config->voltage_sensor_align = DEF_VOLTAGE_SENSOR_ALIGN;    //V alignSensor() use it，大功率电机设置的值小一点比如0.5-1，小电机设置的大一点比如2-3
...
int MiniFoc_pidControllerInit(Motor_t *motor) {
  ...
  PID_init(&motor->P_springWithDump, DEF_P_SPRING_WITH_DUMP_P, DEF_P_SPRING_WITH_DUMP_I, DEF_P_SPRING_WITH_DUMP_D, DEF_SPRING_WITH_DUMP_VEL_LIM, DEF_P_SPRING_WITH_DUMP_RAMP);
  PID_init(&motor->P_dump, DEF_P_DUMP_P, DEF_P_DUMP_I, DEF_P_DUMP_D, DEF_DUMP_VEL_LIM, DEF_P_DUMP_RAMP);
  PID_init(&motor->P_zero_resistance, DEF_P_ZERO_RESISTANCE_P, DEF_P_ZERO_RESISTANCE_I, DEF_P_ZERO_RESISTANCE_D, DEF_ZERO_RESISTANCE_VEL_LIM, DEF_P_ZERO_RESISTANCE_RAMP);
  ...
  PID_init(&motor->PID_current_q, DEF_PID_CURR_Q_P, DEF_PID_CURR_Q_I, DEF_PID_CURR_Q_D, voltage_limit, DEF_PID_CURR_Q_RAMP);
  PID_init(&motor->PID_current_d, DEF_PID_CURR_D_P, DEF_PID_CURR_D_I, DEF_PID_CURR_D_D, voltage_limit, DEF_PID_CURR_D_RAMP);
...
int MiniFoc_init(Motor_t *motor, MotorType_t motorType, char *sensorName, char *driverName) {
  ...
  motor->controller = Type_angle;//Type_velocity;  //Type_torque;  //Type_velocity;  //Type_angle;
  motor->target = 0;
...
int MiniFoc_setController(Motor_t *motor, MotionControlType controller) {
  if (controller < Type_torque || controller >= Type_control_end) {
...
int MiniFoc_setTorque(Motor_t *motor, TorqueControlType torque) {
  if (torque < Type_voltage || torque >= Type_Torque_end) {
