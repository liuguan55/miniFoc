...
void HAL_FLASHEx_OBGetConfig(FLASH_OBProgramInitTypeDef *pOBInit) {
  ...
  /*Get WRP*/
  pOBInit->WRPSector = (uint32_t)
  FLASH_OB_GetWRP();

  /*Get RDP Level*/
  pOBInit->RDPLevel = (uint32_t)
  FLASH_OB_GetRDP();
...
static HAL_StatusTypeDef FLASH_OB_EnableWRP(uint32_t WRPSector, uint32_t Banks) {
  ...
  if (status == HAL_OK) {
    *(__IO
    uint16_t *)OPTCR_BYTE2_ADDRESS &= (~WRPSector);
  }
...
static HAL_StatusTypeDef FLASH_OB_DisableWRP(uint32_t WRPSector, uint32_t Banks) {
  ...
  if (status == HAL_OK) {
    *(__IO
    uint16_t *)OPTCR_BYTE2_ADDRESS |= (uint16_t)WRPSector;
  }
...
static HAL_StatusTypeDef FLASH_OB_UserConfig(uint8_t Iwdg, uint8_t Stop, uint8_t Stdby) {
  ...
  if (status == HAL_OK) {
    ...
    uint8_t *)OPTCR_BYTE0_ADDRESS) &(uint8_t)0x1F);

    /* Update User Option Byte */
    *(__IO
    uint8_t *)OPTCR_BYTE0_ADDRESS = Iwdg | (uint8_t)(Stdby | (uint8_t)(Stop | ((uint8_t)optiontmp)));
  }
...
static HAL_StatusTypeDef FLASH_OB_BOR_LevelConfig(uint8_t Level) {
  ...
  *(__IO
  uint8_t *)OPTCR_BYTE0_ADDRESS |= Level;
