...
static void currentCallback(uint16_t *data, uint32_t size) {
//    log_d("size %d currentCallback %d %d\n",size, data[0], data[1]);
}

static void currentInit(void) {
    HAL_adcInit(HAL_ADC_1);
    HAL_adcSetCallback(HAL_ADC_1, currentCallback);
...
/**
 * @brief  down line current sense driver
 */
static FocCurrent_t currentDriver = {
        .name = "downlineCurrent",
        .data = &currentCtx,
        .init = currentInit,
