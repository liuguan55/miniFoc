...
static unsigned short readAs5600RawData(SensorInterface_t *interface) {
    ...
    HAL_I2CTransmit((HAL_I2C_ID) interface->handle, interface->address << 1, &reg, 1, interface->timeout);
    HAL_I2CReceive((HAL_I2C_ID) interface->handle, interface->address << 1, val, 2, interface->timeout);
...
static void AS5600_init(struct FocSensor *sensor) {
    ...
    I2C_Config i2c_config = {
            .addrMode = HAL_I2C_ADDR_MODE_7BIT,
            .clockSpeed = 100 * 1000,
    };
    HAL_I2CInit(HAL_I2C_1, &i2c_config);
    ctx->cpr = AS5600_CPR;
...
//  printf("angle_data %d\n",ctx->angle_data);
    ...
    if (fabs(ctx->d_angle) > (0.8 * ctx->cpr)) {
        log_d("d_angle %d full_rotation_offset %4.2f angle_data %d", ctx->d_angle, ctx->full_rotation_offset,
              ctx->angle_data);
