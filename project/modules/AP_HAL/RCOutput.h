...
public:
    ...
    /*
      ...
      in the safe state
     */
    virtual void     set_safety_pwm(uint32_t chmask, uint16_t period_us) {UNUSED(chmask);UNUSED(period_us);}

    /*
      set PWM to send to a set of channels if the FMU firmware dies
     */
    virtual void     set_failsafe_pwm(uint32_t chmask, uint16_t period_us) { UNUSED(chmask);UNUSED(period_us);}
