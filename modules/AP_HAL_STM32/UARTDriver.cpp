...
static UART_HandleTypeDef *uartHandles[STM32UARTDriver::HAL_STM32_UART_NR] = {
        &huart1,
        &huart1,
        &huart1,
        &huart1,
...
{
    _huart = uartHandles[uart_num];
    _lwrb = Mini_getlwrbHandle(uart_num);
}
...
size_t STM32UARTDriver::write(const uint8_t *buffer, size_t size)
{
    ...
    if (_writebuf_tail < _head) {
        ...
        MINI_ASSERT(_writebuf_tail+size <= _writebuf_size);
    ...
    if (n > size) n = size;
    MINI_ASSERT(_writebuf_tail+n <= _writebuf_size);
...
int STM32UARTDriver::_write(const uint8_t *buf, uint16_t n) {
    HAL_StatusTypeDef status = HAL_UART_Transmit(_huart, (uint8_t *)buf, n, 1000);
    if (status == HAL_OK) {
